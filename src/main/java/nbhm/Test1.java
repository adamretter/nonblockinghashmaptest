...
public class Test1 {
    ...
    public Test1(final boolean useNBHM, final boolean poolObjects) {
        if (useNBHM) {
    ...
    private void event(final LockEventType lockEventType, final String id, final LockType lockType, final LockMode lockMode) {
        ...
        switch (lockEventType) {
            case Attempt:
//                boolean took = false;
                Entry entry = poolObjects ? THREADLOCAL_ENTRY_POOL.get().takeEntry() : null;
                if (entry == null) {
                    entry = new Entry();
                } //else {
//                    took = true;
...
//            System.out.println("Releasing entry: " + entry.hashCode());
    ...
    private EntryKey key(final long threadId, final String id, final LockType lockType, final LockMode lockMode) {
        ...
        char[] buf = poolObjects ? THREADLOCAL_CHAR_ARRAY_POOL.get().takeEntry() : null;
...
//        boolean took = false;
        EntryKey key = poolObjects ? THREADLOCAL_ENTRY_KEY_POOL.get().takeEntry() : null;
        if (key == null) {
            key = new EntryKey();
        } //else {
//            took = true;
...
//        sendEvents(new Test1(false, false));

        // ConcurrentHashMap with object pooling
//        sendEvents(new Test1(false, true));

        // NonBlockingHashMap with NO object pooling
//        sendEvents(new Test1(true, false));
        ...
        sendEvents(new Test1(true, true));
    }

    private static void sendEvents(final Test1 lockTable) {
        ...
        //1233 [ERROR] LockTable: No entry found when trying to remove acquired attempt for: id=/db/apps/docs/data/2/2, EntryKey.hashCode=540616524

        lockTable.event(LockEventType.Released, "/db/apps/docs/data/2/2", LockType.DOCUMENT, LockMode.READ_LOCK);

        //1239 [ERROR] LockTable: No entry found when trying to release for: id=/db/apps/docs/data/2/2, EntryKey.hashCode=540616524
